...
const changeFriendlistVisibility = async (appUser) => {
    try {
        const url = new URL(`${config.apiUrl}/friend/change-friendlist-visibility`);
        url.searchParams.append('appUser', appUser);
...
const fetchAllFriendRequests = async (appUser) => {
    ...
    try {
        const response = await fetch(url); // GET is default method
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const data = await response.json();
...
export const useChangeFriendlistVisibility = () => {
    const queryClient = useQueryClient();
    return useMutation(changeFriendlistVisibility, {
        onSuccess: () => {
...
export const useAcceptFriendRequest = () => {
    ...
    return useMutation(acceptFriendRequest, {
        onSuccess: () => {
            ...
            // Invalidate and refetch friend requests data
            queryClient.invalidateQueries(['allFriendRequests']);
        },
        onError: (error) => {
            toast.error(`Error: ${error.message}`);
